...
test('recover truncates an incomplete trailing line and append can continue after recovery', async () => { const { journal, file } = await fixture(); const first = await journal.append({ kind: 'checkpoint', scope: { organId: organ, taskId: task }, checkpoint: checkpoint(1, null) }); const line = await readFile(file, 'utf8'); await appendFile(file, line.slice(0, -1), 'utf8'); let result = await journal.verify(); assert.equal(result.valid, false); assert.match(result.error!, /trailing/); const recovered = await journal.recover(); assert.equal(recovered.valid, true); assert.equal(recovered.records.length, 1); assert.equal((await journal.latest())!.seq, first.seq); await journal.append({ kind: 'event', scope: { organId: organ, taskId: task }, payload: { ignored: true } }); assert.equal((await journal.latest())!.seq, 2); });
