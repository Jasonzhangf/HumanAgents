...
- 显式 Brain 负责感知输入整理、任务匹配、状态查询、意图确认和整理反馈；新建任务表单的提交本身就是用户对该新任务的确认，显式 Brain 在同一次提交处理中整理并按 FIFO 投递 `RequirementEnvelope`，不再要求第二次确认。已有任务的目标、范围或处理方式变更仍须单独确认。隐式 Brain 负责分类队列、运行任务更新、资源准入和 Pipeline 创建。steer/停止/权限撤销等控制命令不得混入业务需求队列或 task payload。
